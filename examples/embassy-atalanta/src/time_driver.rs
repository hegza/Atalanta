...
#[cfg(time_driver_tim0)]
type T = bsp::timer_group::Timer0;
#[cfg(time_driver_tim1)]
type T = bsp::timer_group::Timer1;
#[cfg(time_driver_tim2)]
type T = bsp::timer_group::Timer2;
#[cfg(time_driver_tim3)]
type T = bsp::timer_group::Timer3;
...
// Clock timekeeping works with something we call "periods", which are time
// intervals of 2^15 ticks. The Clock counter value is 16 bits, so one "overflow
// cycle" is 2 periods.
//
// A `period` count is maintained in parallel to the Timer hardware `counter`,
// like this:
...
//   0x8000)
//
// Therefore, when `period` is even, counter is in 0..0x7FFF. When odd, counter
// is in 0x8000..0xFFFF This allows for now() to return the correct value even
// if it races an overflow.
//
// To get `now()`, `period` is read first, then `counter` is read. If the
// counter value matches the expected range for the `period` parity, we're done.
// If it doesn't, this means that a new period start has raced us between
// reading `period` and `counter`, so we assume the `counter` value corresponds
// to the next period.
//
// `period` is a 32bit integer, so It overflows on 2^32 * 2^15 / 32768 seconds
// of uptime, which is 136 years.
...
impl RtcDriver {
    ...
    fn on_interrupt(&self) {
        ...
        critical_section::with(|cs| {
            ...
            // Clear all interrupt flags. Bits in SR are "write 0 to clear", so write the
            // bitwise NOT. Other approaches such as writing all zeros, or
            // RMWing won't work, they can miss interrupts.
    ...
    fn next_period(&self) {
        ...
        // We only modify the period from the timer interrupt, so we know this can't
        // race.
    ...
    */
    ...
    fn time_until_next_alarm(&self, cs: CriticalSection) -> embassy_time::Duration {
        ...
        embassy_time::Duration::from_ticks(
            self.alarm.borrow(cs).timestamp.get().saturating_sub(now),
        )
    }
    ...
    fn add_time(&self, offset: embassy_time::Duration, cs: CriticalSection) {
        ...
        let period = if cnt > u16::MAX as u32 / 2 {
            period + 1
        } else {
            period
        };
    ...
    */
    ...
    #[cfg(feature = "low-power")]
    /// The minimum pause time beyond which the executor will enter a low-power
    /// state.
    ...
    fn set_alarm(&self, cs: CriticalSection, timestamp: u64) -> bool {
        ...
        // Write the CCR value regardless of whether we're going to enable it now or
        // not. This way, when we enable it later, the right value is already
        // set.
